...
def write_distance_svg(data, out_path, pairs=None, w=800, h=600):
    ...
    body_steps = {b: {step: (x, y) for (step, x, y) in rows} for b, rows in data.items()}
    ...
    for (a, b) in pairs:
        pts = []
        for s in steps:
            if a in body_steps and b in body_steps and s in body_steps[a] and s in body_steps[b]:
    ...
    for i, ((a, b), pts) in enumerate(sorted(series.items())):
        ...
        if pts_svg:
            lines.append(f'<polyline points="{pts_svg}" fill="none" stroke="{color}" stroke-width="2"/>')
            for s, d in pts:
                lines.append(f'<circle cx="{sx_step(s)}" cy="{sy_d(d)}" r="2" fill="{color}"/>')
        lines.append(f'<text x="{w-240}" y="{30 + i*16}" font-size="12">pair {a}-{b}</text>')

    lines.append(f'<text x="30" y="{h-10}" font-size="12">step</text>')
    lines.append(f'<text x="10" y="30" font-size="12" transform="rotate(-90 10,30)">distance (m)</text>')
    lines.append('</svg>')
...
def main():
    ...
    p.add_argument("-t", "--type", help="Plot type: trajectory or distance", default="trajectory")
    p.add_argument("--pairs", help="Pairs for distance (e.g. '0-1,0-2') or 'all'", default="all")
    args = p.parse_args()
    ...
    if args.type == 'trajectory':
        ...
        write_distance_svg(data, args.out, pairs=(None if args.pairs == 'all' else args.pairs))
